...
int main(int argc, char *argv[]) {
	...
	fixnum_t i, o, m, t, c;
	wordlists_init();
	fixnum_multiplier16_t mul6, mul3273;
	fixnum_multiplier16_init(&mul6, 6);
	fixnum_multiplier16_init(&mul3273, 3273);
	fixnum_init(&i, in, 16);
	...
	for (int i = 0; i < 45; i++) fixnum_mul16(&m, &mul6);
	fixnum_mul16(&m, &mul3273);
	fixnum_show(&m, "max");
	fixnum_show(&i, "input");
